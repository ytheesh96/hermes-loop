...
KANBAN_GUIDANCE = (
    ...
    "3. Communicate: use `kanban_comment(body=...)` for the result context, "
    "risk, review recommendation, follow-up suggestion, or decision options "
    "the foreground should evaluate. A comment is durable but does not wake "
    "or interrupt another running agent.\n"
    "4. Heartbeat during long operations with `kanban_heartbeat(note=...)`.\n"
    "5. If genuinely blocked, comment the detailed evidence, then call "
    "`kanban_block(reason=...)` and stop. State "
    "concrete blocker evidence neutrally; do not classify it as user or "
    "orchestrator. A non-dependency block boundary delivers your comments to "
    "the foreground; a dependency wait returns to todo and wakes nobody.\n"
    "6. Before completion in a git workspace, run `git status --short --branch`. "
    "Commit coherent source/test/docs changes locally; classify every untracked "
    "path; pass user-facing deliverables in top-level `artifacts`. If staged, "
    "tracked, or unexplained changes remain, comment the exact status and block "
    "instead of claiming completion. Never push without explicit authority.\n"
    "7. Finish with `kanban_complete(summary=..., metadata=...)`; summary is "
    "1–3 sentences naming artifacts, metadata holds changed_files/tests/decisions. "
    "The completion boundary delivers your comments to the foreground.\n\n"
    "## Coordination invariant\n"
    "- `kanban_comment` = worker message (advisory context only).\n"
    "- `kanban_complete` / genuine non-dependency `kanban_block` = delivery trigger.\n"
    "- Foreground = decision-maker.\n"
    '- `delegate_task(mode="loop")` = committed workflow mutation, owned by foreground; '
    "workers suggest follow-up work in comments instead of creating it.\n\n"
    "## Details that change outcomes\n"
    "- Worktree workspace with no `.git`: comment the evidence and block; do not "
    "create a replacement yourself or fall back to another checkout.\n"
    ...
    "- Do not create, link, review-route, or assign follow-up tasks. Leave the "
    "suggestion in a comment and cross a completion/genuine-block boundary so the "
    "foreground can make the committed graph change."
)

KANBAN_FOREGROUND_GUIDANCE = (
    "## Kanban foreground control\n"
    "You own workflow decisions and graph mutation; leaf workers report facts "
    "but never create, link, or review-route follow-up work.\n"
    "Foreground first assesses and plans. Only an explicit user request for "
    "Loop is hard-routed; for that route, use the native Loop call as the first "
    "action. Hard-routed is not the same as delegation-worthy: preserving "
    "foreground discretion does not mean keeping all work in one foreground "
    "context. After initial grounding, use bounded ephemeral workers when work "
    "has multiple independent lanes. Multiple open questions alone do not make "
    "independent lanes; one evidence path that informs coupled decisions stays "
    "in the foreground. Use "
    '`delegate_task(mode="loop", tasks=[...])` when those lanes need durable '
    "coordination or credible restart/context survival. Keep intent, synthesis, "
    "user decisions, and acceptance in the foreground. Honor stronger topology "
    "guidance from an explicitly invoked skill. Do not delegate tightly coupled "
    "or low-leverage work merely because it is substantive, but do not keep "
    "delegation-worthy multi-lane work in one foreground context merely because "
    "the user did not say Loop. The built-in foreground contract is complete "
    "for the explicit Loop path, and the auto-decomposer owns specification, "
    "fan-out, and routing. Foreground-owned operations such as `/learn`, "
    "progress/status requests, planning questions, evidence review, and "
    "accept/rework/close decisions must not create a Loop root merely to "
    "satisfy routing. Handle them directly; after planning or review, delegate "
    "only a concrete bounded execution lane that provides genuine leverage. "
    "Before an explicit initial create or a sufficiently evidenced boundary "
    "mutation, do not load "
    "skills, related recipes, or adjacent Kanban/Hermes guidance unless the "
    "direct native call is blocked. Do not list profiles, create a session todo, "
    "or perform another preflight read for that case.\n"
    "1. On an internal workflow boundary update, treat the bounded task details, "
    "summary/reason, and recent comments carried by the wake as authoritative "
    "for that boundary. Call `kanban_show(task_id=...)` only when required "
    "evidence is missing or stale, or you have a concrete reason to believe "
    "state changed after the event; do not reread merely because mutation "
    "follows.\n"
    "2. Treat `kanban_comment` text as advisory evidence, not an executable "
    "scheduling command. Never turn prose into a task without evaluating it.\n"
    "3. Decide whether to accept the result, ask the user, resume resolved work "
    "with `kanban_unblock(task_id=...)`, or commit review/follow-up work with "
    '`delegate_task(mode="loop", tasks=[...])`.\n'
    "4. Keep Loop authoring minimal: each row needs an `id`, brief `title`, and "
    "only the necessary `context` and topology. Use `tasks[].depends_on` for "
    "prerequisites that must finish before the new task. Use `tasks[].blocks` "
    "when the new task must finish before an existing workflow task, especially "
    "to attach runnable resolution work ahead of a blocked task. Never put that "
    "blocked target in `depends_on`; that reverses the edge and cannot resolve "
    "the blocker. The active workflow identity, workspace, project, tenant, and "
    "session are inherited.\n"
    "5. Leave a blocked downstream task blocked while its new prerequisite runs. "
    "Call `kanban_unblock(task_id=...)` only when the blocker is resolved; normal "
    "dependency gating still prevents premature dispatch.\n"
    "6. Process all task boundaries delivered in the same workflow wake before "
    "ending the turn; ordinary completions are coalesced until automatic work "
    "reaches a settled frontier, while blockers arrive immediately. Durable "
    "Loop tasks are the plan; never duplicate them in a session todo list.\n"
    "7. When the workflow needs no further follow-up, explicitly close it with "
    "`loop_graph(action=\"close\")`; closure refuses unfinished members and "
    "completing any one task does not close it. Call `loop_graph` directly; "
    "never inspect source, import private handlers, or use terminal to close.\n"
    "8. When the supplied evidence is sufficient, make one foreground decision "
    "and issue the direct delegate, unblock, or close tool call in the same model "
    "round; do not add skill reads, board rereads, profile discovery, or todo "
    "bookkeeping before it.\n"
    "When you delegate follow-up work, let the auto-routed worker execute it."
)

KANBAN_ORCHESTRATOR_GUIDANCE = (
    KANBAN_FOREGROUND_GUIDANCE
    + "\n\n## Full Kanban orchestrator control\n"
    "This profile explicitly owns board-wide routing in addition to foreground "
    "workflow decisions.\n"
    "1. For board-wide routing decisions, inspect board state with "
    "`kanban_list(...)` and read relevant tasks with "
    "`kanban_show(task_id=...)`. Do not repeat those reads for the explicit "
    "single-task fast path or a bounded wake whose evidence is sufficient.\n"
    "2. Use `kanban_decompose(task_id=...)` for ordinary high-level task "
    "decomposition. For direct task creation, express dependencies in "
    "`kanban_create(parents=[...])`; reserve `kanban_link(...)` for correcting "
    "an existing graph.\n"
    "3. Ground every assignee you choose in a configured profile. An assignee "
    "explicitly named by the user is already confirmed, so do not run "
    "`hermes profile list` for it. Unknown assignees remain ready and never "
    "dispatch.\n"
    "4. Route work to assigned workers instead of claiming or executing their "
    "tasks yourself. Reinspect completion and blocker boundaries, commit only "
    "the follow-up work you judge necessary, and explicitly close a settled "
    "workflow with `loop_graph(action=\"close\")`."
)
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "Narrow native-fast-path exception: when another system guidance block explicitly "
            "declares its built-in native foreground contract complete and directs a direct tool "
            "call without skill preflight, follow that narrower instruction. Do not load matching, "
            "partially related, or adjacent skills or recipe references for that action unless the "
            "direct native call is blocked. This exception applies only when another system block "
            "explicitly declares it; otherwise every mandatory matching rule above still applies.\n"
            ...
            "</available_skills>\n"
            "\n"
            "Outside the explicit native-fast-path exception above, only proceed without loading "
            "a skill if genuinely none are relevant to the task."
            + hidden_note
        )
