...
KANBAN_FOREGROUND_GUIDANCE = (
    ...
    "Fast path: when the user supplies a sufficient task goal and explicitly "
    "names its assignee, treat that profile as confirmed and call `loop_create` "
    "or `kanban_create(...)` directly in the first model round. The built-in "
    "foreground contract is complete for this native Loop path. Before the "
    ...
    "3. Decide whether to accept the result, ask the user, unblock the task, "
    "or commit differently assigned review/follow-up work with "
    "`kanban_create(...)`.\n"
    "4. A profile name explicitly supplied by the user is already confirmed; "
    "never run `hermes profile list` for it. Only when you choose or guess an "
    "assignee must you discover configured profiles or ask the user. Unknown "
    "assignees remain ready and never dispatch.\n"
    "5. For a workflow follow-up after a completed task, use that task in "
    "`parents=[...]`; the new task inherits the hidden workflow identity, "
    "workspace, project, tenant, and session. Never use a blocked task as a "
    "parent: create runnable resolution/review work without parents, reference "
    "the blocked task in its body, and unblock it only after that work resolves. "
    "The active workflow identity is inherited without adding a dependency.\n"
    ...
    "and issue the direct create, unblock, or close tool call in the same model "
    "round; do not add skill reads, board rereads, profile discovery, or todo "
    "bookkeeping before it.\n"
    "Express dependencies in `kanban_create(parents=[...])`; a fully configured "
    "orchestrator may use `kanban_link` only for a later correction. "
    "When you delegate follow-up work, let its assigned worker execute it."
)
