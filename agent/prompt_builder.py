...
KANBAN_GUIDANCE = (
    ...
    "4. **Block on a genuine blocker.** If input, dependency, credentials, "
    "policy, or peer output prevents completion within "
    "worker scope, call `kanban_block(reason=\"...\")` and stop. State "
    "concrete blocker evidence neutrally; do not classify it as user, "
    "foreground, or orchestrator. Ordinary worker blockers are routed into "
    "orchestrator blocker triage on the same task row instead of idling in "
    "blocked; only narrow terminal/system blockers explicitly marked in "
    "metadata may remain non-runnable.\n"
    ...
    "it `needs-user`; foreground review decides if user input is required.\n"
