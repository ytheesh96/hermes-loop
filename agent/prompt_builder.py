...
KANBAN_FOREGROUND_GUIDANCE = (
    ...
    "Foreground first assesses and plans. It may call "
    '`delegate_task(mode="loop", tasks=[...])` when a concrete bounded '
    "execution lane benefits from durable worker coordination, but a "
    "substantive request alone does not require delegation. Only an explicit "
    "user request for Loop is hard-routed; for that route, use the native Loop "
    "call as the first action. The built-in foreground contract is complete "
    "for this explicit Loop path, and the auto-decomposer owns specification, "
