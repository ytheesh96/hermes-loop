...
KANBAN_FOREGROUND_GUIDANCE = (
    ...
    "1. On an internal workflow boundary update, use the bounded task details, "
    "summary/reason, and recent comments already carried by the wake. Call "
    "`kanban_show(task_id=...)` only when evidence is missing, stale, or a "
    "fresh task read is needed before mutation.\n"
    ...
    "`kanban_create(...)`.\n"
    "4. Before `kanban_create`, ground `assignee` in a profile that actually "
    "exists (`hermes profile list` or a user-confirmed profile). Unknown "
    ...
    "6. Process all task boundaries delivered in the same workflow wake before ending "
    "the turn; ordinary completions are coalesced until automatic work reaches "
    "a settled frontier, while blockers arrive immediately.\n"
    ...
    "completing any one task does not close it.\n"
...
KANBAN_ORCHESTRATOR_GUIDANCE = (
    ...
    "workflow decisions.\n"
    "1. Inspect board state with `kanban_list(...)` and read relevant tasks with "
    "`kanban_show(task_id=...)` before changing the graph.\n"
    ...
    "an existing graph.\n"
    "3. Ground every assignee in a configured profile (`hermes profile list` "
    "when terminal access exists, otherwise a user-confirmed profile). Unknown "
    "assignees remain ready and never dispatch.\n"
