...
| `dependency_wait` | `{reason, kind}` | Worker blocked with `kind=dependency` — the task routes to `todo`; only explicit graph parents can release it. Parentless waits are not re-promoted by reconciliation, and repeated waits use the recurrence breaker. |
