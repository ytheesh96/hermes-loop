...
| `kanban` | `kanban_block`, `kanban_comment`, `kanban_complete`, `kanban_create`, `kanban_decompose`, `kanban_heartbeat`, `kanban_link`, `kanban_list`, `kanban_resolve_blocker`, `kanban_show`, `kanban_unblock` | Multi-agent coordination tools. Dispatcher-spawned workers (`HERMES_KANBAN_TASK`) receive their scoped lifecycle controls. Enabled Loop origins retain the bounded foreground re-entry controls (`show`, `complete`, `comment`, `create`, `unblock`), including in TUI coding focus mode. Profiles that explicitly list `kanban` receive the full orchestrator surface. Workers cannot mutate the graph. |
