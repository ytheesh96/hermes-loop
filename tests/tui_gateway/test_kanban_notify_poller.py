...
``_collect_tui_kanban_notifications`` (lease claim + formatting),
``_finalize_tui_kanban_claims`` (ACK + terminal cleanup), and
``_format_tui_kanban_notification``.
"""
...
from tui_gateway.server import (
    _collect_tui_kanban_notifications,
    _finalize_tui_kanban_claims,
    _format_tui_kanban_notification,
)
...
def _collect_and_ack(session: dict) -> list[str]:
    claims: list[dict] = []
    texts = _collect_tui_kanban_notifications(session, claims_out=claims)
...
class TestCollectKanbanNotifications:
    def test_delivers_completed_event_and_unsubscribes(self):
        ...
        texts = _collect_and_ack(_session())
    ...
    def test_claim_advances_cursor_so_second_poll_is_empty(self):
        ...
        first = _collect_and_ack(_session())
        second = _collect_and_ack(_session())
    ...
    def test_no_session_key_is_a_noop(self):
        ...
        assert _collect_and_ack({"session_key": ""}) == []
        assert _collect_and_ack({"session_key": None}) == []
        assert len(_sub_rows(tid)) == 1
    ...
    def test_profile_scoped_session_reads_the_shared_board(self, tmp_path):
        ...
        try:
            texts = _collect_and_ack(session)
        finally:
...
class TestFormatKanbanEventText:
    ...
    def test_silent_kinds_return_none(self):
        for kind in ("archived", "unblocked"):
            ...
            assert _format_tui_kanban_notification(self.SUB, self.TASK, ev) is None

    def test_blocked_includes_reason(self):
        ev = SimpleNamespace(kind="blocked", payload={"reason": "needs creds"})
        text = _format_tui_kanban_notification(self.SUB, self.TASK, ev)
...
class TestNotificationPollerLoopKanbanWiring:
    ...
    Covers the wiring above ``_collect_tui_kanban_notifications``:
    status.update emission, agent-turn dispatch when the session is idle, and
    leaving the durable lease untouched until a busy session goes idle.
    """
    ...
    def _start_poller(self, session: dict, monkeypatch):
        ...
        dispatch = server._dispatch_tui_kanban_notifications
        ...
        monkeypatch.setattr(
            server,
            "_dispatch_tui_kanban_notifications",
            track_dispatch,
        )
        ...
        thread.start()
        return stop, thread, emits, submits, attempts
    ...
    def test_idle_session_gets_status_update_and_agent_turn(self, monkeypatch):
        ...
        stop, thread, emits, submits, _attempts = self._start_poller(
            session,
            monkeypatch,
        )
        try:
    ...
    def test_busy_session_leaves_event_unclaimed_then_delivers_when_idle(
        self,
        monkeypatch,
    ):
        tid = _create_subscribed_task()
        ...
        pre_cursor = int(_sub_rows(tid)[0]["last_event_id"])
        stop, thread, emits, submits, attempts = self._start_poller(
            session,
            monkeypatch,
        )
        try:
            assert self._wait_for(lambda: attempts)
            assert not submits
            assert not emits
            assert int(_sub_rows(tid)[0]["last_event_id"]) == pre_cursor

            with session["history_lock"]:
                session["running"] = False

            assert self._wait_for(lambda: submits), "durable event was never delivered"
        finally:
            stop.set()
            thread.join(timeout=5)
