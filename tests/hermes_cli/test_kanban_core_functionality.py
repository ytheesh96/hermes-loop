...
def test_gateway_dispatcher_disables_corrupt_board_without_traceback(
    ...
    async def _to_thread(fn, *args, **kwargs):
        # One tick is reaper + one board enumeration + dispatch. Let two
        # complete so the second exercises the corruption quarantine.
        calls["to_thread"] += 1
    ...
    # The first tick opens once for dispatch. Health is derived from that same
    # connection, and the quarantined second tick opens nothing.
...
def test_dispatch_once_integrates_stale_detection(kanban_home, monkeypatch):
    ...
    signals = []
    monkeypatch.setattr(_kb.os, "kill", lambda pid, sig: signals.append((pid, sig)))
    ...
    with kb.connect() as conn:
        ...
        assert signals and signals[0][0] == 99999
...
def test_duplicate_notify_sub_preserves_existing_cursor(kanban_home):
    """Re-registering a route must not replay events it already consumed."""
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="idempotent route", assignee="w")
        kb.add_notify_sub(
            conn,
            task_id=tid,
            platform="telegram",
        ...
        assert sub["scope"] == "descendants"
        _, replayed = kb.unseen_events_for_sub(
            conn,
            task_id=tid,
            platform="telegram",
            chat_id="123",
