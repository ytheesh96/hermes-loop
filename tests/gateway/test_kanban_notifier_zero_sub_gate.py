...
def test_workflow_only_board_is_opened_through_the_gate(tmp_path, monkeypatch):
    """A workflow route must count even when no task subscription exists."""
    db_path = tmp_path / "workflow-only.db"
    monkeypatch.setenv("HERMES_KANBAN_DB", str(db_path))
    kb.init_db()
    conn = kb.connect()
    try:
        workflow_id = kb.create_workflow(conn, workflow_id="wf-only")
