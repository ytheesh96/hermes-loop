...
def test_session_resume_uses_compression_tip_for_agent_history(monkeypatch):
    ...
    class FakeDB:
        ...
        def get_messages_as_conversation(
            ...
            captured.setdefault("history_calls", []).append(
                (target, include_ancestors, include_compression_lineage, repair_alternation)
            )
    ...
    assert captured["history_calls"] == [
        ("tip", False, False, True),
        ("tip", True, False, False),
    ]
...
def test_async_delegation_dispatch_keeps_display_metadata(monkeypatch):
    ...
    assert captured["display_kind"] == "async_delegation_complete"
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with (
            ...
            patch(
                ...
                side_effect=AssertionError("test attempted to launch a real browser"),
            ),
            patch("hermes_cli.browser_connect.local_port_in_use", return_value=False),
        ):
            resp = server.handle_request(
...
def test_tui_kanban_dispatch_failure_rewinds_then_success_cleans_subscription(
    ...
    try:
        ...
        initial_cursor = int(
            kb.list_notify_subs(conn, task_id)[0]["last_event_id"]
        )
    ...
    try:
        ...
        assert int(subscriptions[0]["last_event_id"]) == initial_cursor
...
def test_tui_kanban_collect_respects_profile_for_descendant_block(monkeypatch, tmp_path):
    ...
    try:
        ...
        initial_cursor = int(
            kb.list_notify_subs(conn, root)[0]["last_event_id"]
        )
