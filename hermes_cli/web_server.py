...
async def get_session_messages(
    ...
    def _read():
        ...
        try:
            ...
            # Clamp limit to prevent abuse (max 500 per page), then page the
            # lineage-aware display transcript assembled above.
            _limit = min(limit, 500) if limit is not None else None
            messages = messages[offset:offset + _limit] if _limit is not None else messages[offset:]
