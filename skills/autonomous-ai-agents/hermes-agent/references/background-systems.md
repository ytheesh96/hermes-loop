...
see a focused `kanban_*` toolset gated by `HERMES_KANBAN_TASK`. Enabled
Loop foregrounds retain only the controls needed to handle workflow
wakes, while orchestrator profiles can opt into the full toolset.
...
- **Task worker:** `kanban_show`, `kanban_complete`, `kanban_block`,
  `kanban_heartbeat`, `kanban_comment`, plus task attachment upload/list
  tools.
- **Enabled Loop foreground:** `kanban_show`, `kanban_complete`,
  `kanban_comment`, and `kanban_unblock`.
- **Configured orchestrator:** the full worker and foreground surface,
  plus `kanban_list`, `kanban_create`, `kanban_link`, and
  `kanban_decompose`.
