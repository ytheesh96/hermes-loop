...
function createSessionWindow(sessionId, { watch = false, profile } = {}) {
...
ipcMain.handle('hermes:fs:gitDiff', async (_event, filePath) => {
  const root = await gitRootForIpc(filePath)

  if (!root) {
    return { diff: '', error: 'git-root-unavailable', path: filePath, root: null }
  }

  const diff = await fileDiffVsHead(root, filePath, resolveGitBinary())

  return { diff: diff || '', path: filePath, root }
})

ipcMain.handle('hermes:fs:worktrees', async (_event, cwds) => {
  const out = {}
