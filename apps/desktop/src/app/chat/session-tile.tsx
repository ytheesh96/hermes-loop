...
function useTileMenuRow(
  ...
  return useSyncExternalStore(subscribe, () => {
    ...
    const profile = tile?.profile ?? stored?.profile
    const readOnly = tile?.watch === true
    const key = `${pinId}\u0000${title}\u0000${profile ?? ''}\u0000${readOnly}`

    if (cache.current?.key !== key) {
      cache.current = { key, value: { pinId, profile, readOnly, title } }
    }
...
export function SessionTabMenu({
  ...
  const { pinId, profile, readOnly, title } = useTileMenuRow(storedSessionId)
