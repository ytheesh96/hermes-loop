...
function useSessionActions({
  ...
  const openItems: ActionItemSpec[] = readOnly
    ? []
    : [
          ...
          ? [
              spec({
                ...
                onSelect: () => {
                  ...
                  // sticky-shows on gain) — the door to the tab bar. Focuses
                  // first if the session is already on screen.
  ...
  const identityItems: ActionItemSpec[] = readOnly
    ? []
    : [
        spec({
          disabled: !sessionId,
  ...
  const workItems: ActionItemSpec[] = [
    ...
    ...(!readOnly
      ? [
          spec({
            disabled: !onBranch,
            // Match the inline message action's GitFork glyph. This codicon
            // font has no `git-fork`; `repo-forked` is the fork icon.
            icon: 'repo-forked',
            label: r.branchFrom,
            onSelect: () => {
              triggerHaptic('selection')
  ...
  const dangerItems: ActionItemSpec[] = readOnly
    ? []
    : [
        spec({
          disabled: !onArchive,
...
export function SessionActionsMenu({ children, align = 'end', sideOffset = 6, ...actions }: SessionActionsMenuProps) {
  ...
  return (
    <>
      <ActionsMenu
        align={align}
        ariaLabel={`Actions for ${actions.title}`}
...
export function SessionContextMenu({ children, ...actions }: SessionContextMenuProps) {
  ...
  return (
    <>
      <ActionsContextMenu ariaLabel={`Actions for ${actions.title}`} contentClassName="w-40" items={renderItems}>
        {children}
      </ActionsContextMenu>
      {renameDialog}
    </>
