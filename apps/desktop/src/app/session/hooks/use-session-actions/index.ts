...
export function useSessionActions({
  ...
  const resumeSession = useCallback(
    async (storedSessionId: string, replaceRoute = false) => {
      ...
      const sessionProfile = storedForProfile?.profile ?? sessionWindowProfile()
      ...
      try {
        ...
        // A lazy watch resume can bind the live runtime without returning its
        // durable transcript. Recover that history through REST before folding
        // in the crash journal so a journal tail can never masquerade as the
        // complete conversation.
        if ((watchWindow || sessionShouldHaveTranscript(stored)) && preferredMessages.length === 0) {
          try {
            ...
            preferredMessages = preserveLocalAssistantErrors(toChatMessages(fallback.messages), currentMessages)
          } catch {
            // The failure latch below handles the still-empty state.
          }
        }
