  ...
  const resumeSession = useCallback(
    async (storedSessionId: string, replaceRoute = false) => {
      ...
      try {
        ...
        if ((watchWindow || sessionShouldHaveTranscript(stored)) && messagesForView.length === 0) {
          try {
            ...
            messagesForView = preserveLocalAssistantErrors(preferredMessages, currentMessages)
          } catch {
