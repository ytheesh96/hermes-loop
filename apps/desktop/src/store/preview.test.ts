...
describe('preview store', () => {
  ...
  it('opens file-browser html in rendered preview mode when requested', () => {
    const target = fileTarget('/work/demo.html')

    openPreview(target, 'file-browser', { mode: 'preview' })

    expect($previewTarget.get()).toEqual({ ...target, renderMode: 'preview' })
  })

  it('opens file-browser html in source mode when requested', () => {
    const target = fileTarget('/work/demo.html')

    openPreview(target, 'file-browser', { mode: 'source' })

    expect($previewTarget.get()).toEqual({ ...target, renderMode: 'source' })
  })
