...
test.describe('scoped task Messages across profile backends', () => {
  ...
  test('renders immutable Buzz-style threads and retains a stale snapshot after cold reopen', async ({}, testInfo) => {
    ...
    const artifact = path.join(sandbox.hermesHome, 'report.pdf')
    ...
    await expect(rawPathControl).not.toHaveClass(/border-border|bg-card|bg-background|px-|py-|rounded/)
    const explicitPreview = feed.getByRole('button', { name: 'Preview: report.pdf', exact: true })
    await expect(explicitPreview).toBeVisible()
    await expect(explicitPreview).toHaveAttribute('data-preview-target', artifact)
    await expect(explicitPreview).not.toHaveClass(/border-border|bg-card|bg-background|px-|py-|rounded/)
    ...
    await page.screenshot({ path: testInfo.outputPath('raw-filepath-preview-open.png'), fullPage: true })
    ...
    await page.screenshot({ path: testInfo.outputPath('raw-filepath-normal.png'), fullPage: true })
    const missingPathControl = feed.locator(`[data-preview-target="${missingFixture}"]`)
    await missingPathControl.click()
    await expect(page.getByRole('alert').getByText('Preview unavailable', { exact: true }).first()).toBeVisible()
    ...
    await page.screenshot({ path: testInfo.outputPath('raw-filepath-640px.png'), fullPage: true })
