...
class SessionDB:
            ...
            with self._lock:
                cursor = self._conn.execute(
                    ...
                    ORDER BY
                      ...
                      s.id DESC
                    LIMIT 1
                    """.format(compression_child=_COMPRESSION_CHILD_SQL.format(a="s")),
                    (current,),
                )
                row = cursor.fetchone()
            if row is None:
    ...
    def _compact_session_cols(cls) -> str:
        """SELECT list for compact rows, excluding the system-prompt blob."""
    ...
    def get_messages_as_conversation(
        ...
        return self._rows_to_conversation(
            ...
            include_ancestors=len(session_ids) > 1,
