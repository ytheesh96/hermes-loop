...
def _resume_conversations_for_agent(
    db: Any, session_id: str
) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Load compacted model and full display histories for resume.
    ...
    """
    def _load(target: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        get_both = getattr(db, "get_resume_conversations", None)
        if callable(get_both):
            return get_both(target)
        return (
            db.get_messages_as_conversation(target, repair_alternation=True),
...
def _(rid, params: dict) -> dict:
    ...
    The same detection the coding-context posture bakes into the system prompt,
    exposed so UIs consume it instead of re-sniffing. ``{"facts": null}``
    means the cwd isn't a code workspace.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    if not is_truthy_value(params.get("eager_build", False)):
        ...
        try:
            ...
            raw_history, display_history = _resume_conversations_for_agent(db, target)
        except Exception as e:
            if lease is not None:
                lease.release()
            return _err(rid, 5000, f"resume failed: {e}")
            ...
            try:
                try:
                    ...
                    if "unexpected keyword argument" not in message or (
                        "cwd" not in message
                        and "session_db" not in message
                        and "source" not in message
                    ):
                        raise
...
def _dispatch_notification_text(
    ...
    *,
    ...
    if event is not None:
        ...
        delivery_claim = claim_event_delivery(event, delivery_consumer)
    ...
    try:
        ...
        if event is not None and event.get("type") == "async_delegation":
            prompt_kwargs["display_kind"] = "async_delegation_complete"
            prompt_kwargs["display_metadata"] = (
                _async_delegation_display_metadata(event)
            )
        _run_prompt_submit(
            rid,
            sid,
...
def _notification_poller_loop(
    ...
    Also drains this session's TUI kanban subscriptions through
    ``_dispatch_tui_kanban_notifications`` and delivers terminal task events
    the same way (status.update + agent turn) — the delivery path
    ...
    while not stop_event.is_set() and not session.get("_finalized"):
        # TUI kanban delivery runs through _dispatch_tui_kanban_notifications
        # below (claim lifecycle + workflow routes), not an inline poll here.
        try:
