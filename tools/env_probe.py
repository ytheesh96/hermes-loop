...
def get_environment_probe_line(*, force_refresh: bool = False) -> str:
    ...
    """
    global _CACHED_LINE, _PROBE_DONE, _PROBE_THREAD, _PROBE_GEN
    global _WAIT_ALREADY_TIMED_OUT
    if force_refresh:
        with _CACHE_LOCK:
            old_done = _PROBE_DONE
            _CACHED_LINE = None
            _PROBE_DONE = threading.Event()
            _PROBE_THREAD = None
            _PROBE_GEN += 1
            _WAIT_ALREADY_TIMED_OUT = False
            # Wake callers waiting on the superseded generation. They fail
            # open while the refreshed generation starts independently.
            old_done.set()
...
def _ensure_probe_started() -> None:
    ...
    global _CACHED_LINE, _PROBE_THREAD
    with _CACHE_LOCK:
        if _PROBE_DONE.is_set():
            return
        ...
        try:
            ...
            # Interpreter shutdown can reject new threads. The probe line is
            # optional, so cache an empty result instead of failing startup.
            _PROBE_THREAD = None
            _CACHED_LINE = ""
            _PROBE_DONE.set()
